...
if k_number == 3:
    model_path = f'./save_model/transformer_model_train_ikeda_mu0.97_bifurcation_{k_number}_iter0.pth'
else:
    model_path = f'./save_model/transformer_model_train_ikeda_mu0.97_bifurcation_{k_number}_iter0.pth'
data_dir = './save_data'
...
if k_number == 3:
    ...
    ks = np.arange(0.91, 0.97+1e-12, 0.002)
    ks = np.round(ks, 3)
    
systems_train = []
for k in ks:
    systems_train.append(f'ikeda_mu{k}')

systems_test = ['ikeda_mu1.01']

dim = 2
input_size = 3 # 3 states + k channel (matching training file)
output_size = 2
hidden_size = 128
nhead = 2
...
dropout = 0.2
sequence_length = 512
start_point_test = 30 # 1600 - sequence_length # 20 
...
# Print results (matching training file style)
...
# with open(f'./save_for_plot/transformer_test_ikeda_bifurcation_{k_number}.pkl', 'wb') as f:
#     pickle.dump(results, f)

# with open(f'./save_for_plot/transformer_test_ikeda_bifurcation_{k_number}_example1.pkl', 'wb') as f:
#     pickle.dump(results, f)
